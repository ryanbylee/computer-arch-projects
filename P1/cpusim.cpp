...
int main(int argc, char* argv[])
{
	...
	*/
	...
	while (done == true) // processor's main loop. Each iteration is equal to one clock cycle.  
	{
		...
		myCPU.setController(&instr);

		int ALURes = myCPU.Execute(&instr);

		int MemData = myCPU.MemRW(ALURes, &instr);

		myCPU.WriteBack(&instr, ALURes, MemData);
		myCPU.nextPCCalc(ALURes, &instr);
		myCPU.resetController();
	...
	int a0 =myCPU.regs[10];
	int a1 =myCPU.regs[11];  
